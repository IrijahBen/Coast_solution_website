# CoastApp — Architecture Documentation

**Supply chain finance, transparently secured.**

This document describes the CoastApp application architecture: what it does, which tools and Stellar-based projects it uses, and how the pieces fit together. Diagrams use [Mermaid](https://mermaid.js.org/) and render in GitHub, GitLab, and most Markdown viewers.
...
flowchart TB
  ...
  subgraph App["CoastApp Application"]
...
| Env var | Purpose |
|---------|---------|
| `NEXT_PUBLIC_CoastApp_PLATFORM_ADDRESS` | Platform Stellar address — used as `releaseSigner`, `disputeResolver`, and `platformAddress` in escrow roles |
...
CoastApp supports **multiple ramp providers**. Users choose one in the UI; the app proxies all anchor calls through **API routes** so API keys stay server-side.
...
| `NEXT_PUBLIC_CoastApp_PLATFORM_ADDRESS` | Public | Platform Stellar address (escrow roles) |
...
```mermaid
flowchart TB
  subgraph What["What CoastApp does"]
